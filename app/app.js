// ================================================================
// IPTV — app.js v7.1 | Samsung TV 2025 / TizenBrew
// ================================================================
// Fixes v7.1:
// • All missing closing braces fixed (parse error)
// • Jio API endpoints corrected (tv.media.jio.com OTP flow)
// • Jio channels/stream endpoints corrected
// • Stale stream URL cleared before each Jio channel play
// • ColorF0Red tab cycle off-by-one fixed
// • showFavourites hoisting fixed
// • Jio auth headers corrected
// ================================================================
...
const FAV_IDX      = 2;
const JIO_IDX      = 3;
const TOTAL_TABS   = PLAYLISTS.length + 2; // 4 tabs total
...
/* ── Jio TV API constants ────────────────────────────────── */
// Correct endpoints from JioTV Go v3 source (jiotv-go/jiotv_go)
...
const JIO_STREAM_PATH  = 'https://jiotvapi.media.jio.com/playback/apis/v1.1/geturl?langId=6';
const JIO_REFRESH_URL  = 'https://auth.media.jio.com/tokenservice/apis/v1/refreshtoken?langId=6';

function jioHeaders(token) {
  const h = {
  ...
  if (token) {
    h['accessToken']      = token.accessToken;
    h['ssoToken']         = token.ssoToken;
    h['subscriberId']     = token.subscriberId || '';
    h['uniqueId']         = token.uniqueId     || '';
    h['crm']              = token.crm          || '';
...
/* ── showFavourites (defined before toggleFav uses it) ───── */
...
function toggleFav(ch){
  const key = ch.url || ch.channelId;
  favSet.has(key) ? favSet.delete(key) : favSet.add(key);
  saveFavs();
...
const VS = {
  ...
  _build(i){
    ...
    const logo = ch.logo
      ? `<div class="ch-logo"><img src="${esc(ch.logo)}" alt="" onerror="this.style.display='none';this.nextSibling.style.display='flex'"/><span class="ch-logo-fb" style="display:none">${ini}</span></div>`
...
function applySearch(){
  ...
  sdTm = setTimeout(() => {
    ...
    // Use correct channel source depending on active tab
    const src = plIdx === JIO_IDX ? jioChannels : plIdx === FAV_IDX ? allChannels.filter(c => favSet.has(c.url || c.channelId)) : channels;
...
function startPreview(idx){
  ...
  // Jio channel — always clear cached URL so we re-fetch a fresh stream (avoids expired URLs)
  if(ch.channelId){
    ch.url = null; // clear stale stream URL every time
    if(!jioToken || jioToken.expires <= Date.now()){
      setStatus('Jio session expired — please re-login', 'error');
      showJioLoginModal();
      return;
    }
    ...
    jioGetStreamUrl(ch.channelId)
      ...
      .catch(err => { finishLoadBar(); setStatus('Jio stream error: ' + err.message, 'error'); console.error(err); });
    return;
  }
...
function doPlayPreview(ch, idx){
  ...
  const onMetadata = () => { video.removeEventListener('loadedmetadata', onMetadata); applyAutoAspect(); };
  ...
  try{
    ...
    finishLoadBar(); setStatus('Play error','error');
  }
}
...
window.addEventListener('keydown', e => {
  ...
  // Colour buttons
  // FIX: was % (PLAYLISTS.length+1) — missed Jio tab. Now % TOTAL_TABS (4)
...
/* ================================================================
   JIO TV INTEGRATION — Fixed API (v7.1)
   Endpoints from: jiotv-go/jiotv_go v3 source
================================================================ */

/**
 * Send OTP to mobile number.
 * Endpoint: POST https://tv.media.jio.com/apis/v2.0/loginotp/sendotp
 */
async function jioRequestOtp(mobile){
  const res = await fetch(`${JIO_OTP_BASE}/sendotp`, {
    method: 'POST',
    headers: jioHeaders(null),
    body: JSON.stringify({ number: mobile }),
    mode: 'cors', credentials: 'omit',
  });
  if(!res.ok) throw new Error(`OTP request failed: HTTP ${res.status}`);
  const data = await res.json();
  // API returns { OTPId: "...", ... } or { otpId: "..." }
 ...
 * Endpoint: POST https://tv.media.jio.com/apis/v2.0/loginotp/verifyotp
 */
async function jioVerifyOtp(mobile, otp, otpId){
  const res = await fetch(`${JIO_OTP_BASE}/verifyotp`, {
    method: 'POST',
    headers: jioHeaders(null),
    body: JSON.stringify({ number: mobile, otp: otp, otpId: otpId }),
    mode: 'cors', credentials: 'omit',
  });
  if(!res.ok) throw new Error(`OTP verify failed: HTTP ${res.status}`);
  const data = await res.json();
  // Response fields: authToken (accessToken), ssoToken, refreshToken,
  //                  sessionAttributes.user: { subscriberId, uniqueId }
  const accessToken   = data.authToken   || data.access_token  || data.accessToken;
  const ssoToken      = data.ssoToken    || data.sso_token;
  const refreshToken  = data.refreshToken|| data.refresh_token;
  const expiresIn     = data.tokenExpiry || data.expires_in || 3600;
  const userAttrs     = (data.sessionAttributes && data.sessionAttributes.user) || {};
  if(!accessToken) throw new Error(data.message || 'Login failed — no token');
  return {
    accessToken, ssoToken: ssoToken || accessToken,
    refreshToken, expiresIn,
    subscriberId: userAttrs.subscriberId || '',
    uniqueId:     userAttrs.uniqueId     || '',
    crm:          userAttrs.subscriberId || '',
  };
}

/**
 * Refresh an expired access token.
 * Endpoint: POST https://auth.media.jio.com/tokenservice/apis/v1/refreshtoken
 */
async function jioRefreshToken(){
  if(!jioToken || !jioToken.refreshToken) throw new Error('No refresh token');
  const res = await fetch(JIO_REFRESH_URL, {
    method: 'POST',
    headers: {
      ...jioHeaders(jioToken),
      'refreshToken': jioToken.refreshToken,
    },
...
async function jioEnsureToken(){
  ...
  if(jioToken.expires && jioToken.expires - Date.now() < 120000){
    ...
    catch(e){ console.warn('Token refresh failed, will try anyway:', e.message); }
  }
}

/**
 * Fetch all channels from JioTV CDN.
 * Endpoint: GET https://jiotv.data.cdn.jio.com/apis/v3.0/getMobileChannelList/get/
 */
async function jioGetAllChannels(){
  await jioEnsureToken();
  const res = await fetch(JIO_CHANNELS_URL, {
    headers: jioHeaders(jioToken),
    mode: 'cors', credentials: 'omit',
  });
  if(!res.ok) throw new Error(`Channels fetch failed: HTTP ${res.status}`);
  const data = await res.json();
  // Response: { result: [ { channel_id, channel_name, logoUrl, ... }, ... ] }
  const list = data.result || data.channels || data.channel_list || [];
  return list;
}

/**
 * Get playback stream URL for a channel.
 * Endpoint: GET https://jiotvapi.media.jio.com/playback/apis/v1.1/geturl?langId=6&channelId=X&quality=HD
 */
async function jioGetStreamUrl(channelId, quality){
  await jioEnsureToken();
  quality = quality || 'HD';
  const url = `${JIO_STREAM_PATH}&channelId=${channelId}&quality=${quality}`;
  const res = await fetch(url, {
    headers: jioHeaders(jioToken),
    mode: 'cors', credentials: 'omit',
  });
  if(!res.ok) throw new Error(`Stream URL failed: HTTP ${res.status}`);
  const data = await res.json();
  const streamUrl = data.result || data.url || data.stream_url;
  if(!streamUrl) throw new Error(data.message || 'No stream URL in response');
  return streamUrl;
}

/**
 * Load all Jio channels into the list.
 */
async function loadJioChannels(){
  ...
  try{
    ...
    startLoadBar();
    const list = await jioGetAllChannels();
    jioChannels = list.map(ch => ({
      ...
      url:       null, // always fetch fresh on play
    }));
...
function showJioLoginModal(){
  ...
  reqBtn.onclick = async () => {
    ...
    try{
      ...
      statusDiv.innerHTML = '<span style="color:#e50000">Error: ' + err.message + '</span>';
  ...
  verifyBtn.onclick = async () => {
    ...
    try{
      const tokenData = await jioVerifyOtp(mobile, otp, otpId);
      jioToken = {
...
async function initJio(){
  ...
  try{
    ...
    // Accept stored token if not expired (or if refresh token present for renewal)
    if(t.accessToken && (t.expires > Date.now() || t.refreshToken)){
      jioToken = t;
      if(t.expires <= Date.now()){
        // Token expired but we have refresh — try to renew silently
        try{ await jioRefreshToken(); }
        catch(e){ localStorage.removeItem('jioToken'); jioToken = null; return; }
      }
      // Only auto-load if Jio tab was the last active tab
      if(plIdx === JIO_IDX) await loadJioChannels();
    } else {
      localStorage.removeItem('jioToken');
    }
  } catch(e){ localStorage.removeItem('jioToken'); }
}
