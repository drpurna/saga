// ================================================================
// SAGA IPTV — app.js v33.0  |  Samsung Tizen OS9  |  2025 TV
// All 10 bug fixes + LAN auto-scan JioTV + manual entry fallback
// ================================================================
'use strict';
...
var VS_POOL_MAX          = 60;    // FIX #6: cap pool to prevent unbounded growth
var AV_SYNC_STEP         = 50;
var AV_SYNC_MAX          = 500;
var MAX_RECONNECT        = 5;
var STALL_TIMEOUT        = 12000; // FIX #2: raised from 9000ms for long-GOP live streams
var OVERLAY_AUTO_HIDE    = 4000;
var SEARCH_DEBOUNCE_MS   = 300;
var JP_PLAY_TIMEOUT_MS   = 15000; // FIX #1: global spinner timeout
var EPG_INTERVAL_MS      = 30000; // FIX #4: 30s EPG polling (was 800ms)
...
var _fsExitFallback  = null; // FIX #7: fallback timer for fullscreen exit
...
// JioTV connect state (for scan UI in modal)
...
// ── M3U parser — robust quoted-attribute handling (FIX #8) ────────
function parseM3U(text) {
  var lines  = String(text || '').split(/\r?\n/);
  var out    = [], meta = null;
  // Robust attribute parser: handles any order, quoted values
  var _attr  = function (line, name) {
    var re = new RegExp(name + '\\s*=\\s*"([^"]*)"', 'i');
    var m  = line.match(re); return m ? m[1] : '';
  };
  var _cleanName = function (raw) {
    return String(raw||'')
      .replace(/\s*\([^)]*\)|\s*\[[^\]]*\]/g,' ')
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('#EXTINF')) {
      // Name is always after the LAST comma on the #EXTINF line
      var commaIdx = line.lastIndexOf(',');
      var rawName  = commaIdx >= 0 ? line.slice(commaIdx + 1).trim() : 'Unknown';
      meta = {
        name:  _cleanName(rawName) || rawName,
...
// ── Audio track — event-driven, persisted (FIX #5) ────────────────
function loadPreferredAudioLang() {
  _preferredAudioLang = lsGet(PREF_AUDIO_KEY) || null;
}
function _hookAudioReady() {
  // FIX #5: use loadedmetadata/canplay instead of setTimeout
  if (!Dom.video) return;
  var applied = false;
  function tryApply() {
    if (applied || !_preferredAudioLang) return;
...
// ── Sleep timer — only counts while video is playing (FIX #3) ─────
// FIX #3: only reset on navigation/play keys, not volume/mute/info
var _SLEEP_RESET_KEYS = new Set([
  KEY.UP,KEY.DOWN,KEY.LEFT,KEY.RIGHT,KEY.ENTER,
  KEY.PLAY,KEY.PAUSE,KEY.PLAY_PAUSE,KEY.STOP,
  KEY.CH_UP,KEY.CH_DOWN,KEY.FF,KEY.RW,
...
function setSleepTimer(m) {
  _clearSleepState(); sleepMinutes=m;
...
function _startSleepCountdown(ms) {
  ...
  sleepTimer=setInterval(function(){
    if (!Dom.video||Dom.video.paused){sleepLastTick=Date.now();return;} // freeze while paused
    var now=Date.now(), elapsed=now-sleepLastTick; sleepLastTick=now; sleepRemainingMs-=elapsed;
    if(sleepRemainingMs<=0){
      _clearSleepState(); SagaPlayer.stop(); stopStallWatchdog();
      setStatus('Sleep — stopped','idle'); showToast('Goodnight!',4000);
    }
  },1000);
}
function resetSleepTimer(kc) {
  if (!sleepMinutes||!sleepTimer) return;
  if (kc!==undefined && !_shouldResetSleep(kc)) return;
...
function _clearSleepState(){if(sleepTimer){clearInterval(sleepTimer);sleepTimer=null;}sleepRemainingMs=0;}

// ── Stall watchdog — FIX #2: also listens to playing + progress ───
...
if (Dom.video) {
  // FIX #2: reset stall timer on playing, progress, AND timeupdate
  ['timeupdate','playing','progress'].forEach(function(ev){
    Dom.video.addEventListener(ev, function(){ if(!Dom.video.paused) lastPlayTime=Date.now(); });
  });
}
...
// ══════════════════════════════════════════════════════════════════
// VIRTUAL SCROLL — FIX #6: pool cap + explicit DOM removal
// ══════════════════════════════════════════════════════════════════
var VS = {
  ...
  setData: function(n){
    ...
    for(var k in this.nodes){
      ...
      // FIX #6: cap pool; discard excess nodes from DOM entirely
  ...
  paint: function(){
    ...
    for(var oi in this.nodes){
      ...
      if(ii<s||ii>e){
        ...
        // FIX #6: only pool if under cap, else remove from DOM
        if(this.pool.length<VS_POOL_MAX){this.pool.push(nd);}
        else if(nd.parentNode){nd.parentNode.removeChild(nd);}
...
// ── Search — FIX: debounce properly cancels prior request ─────────
var _sdTm=null;
...
// ── M3U playlist load — FIX #9: quota toast + auto-evict ─────────
function loadPlaylist(urlOv){
  ...
  cacheP.then(function(cached){
    if(cached&&cached.length>100){_onLoaded(cached,true);return;}
    setStatus('Loading…','loading');startLoadBar();
    xhrFetch(rawUrl,30000,function(err,text){
      if(!err&&text&&text.length>100){
        finishLoadBar();
        // FIX #9: check quota before saving, warn user
        if(window.AppCache){
          if(AppCache.lsNearQuota()){showToast('Storage nearly full — clearing old cache',3500);AppCache.clearAllM3U().then(function(){AppCache.setM3U(rawUrl,text);});}
          else AppCache.setM3U(rawUrl,text);
        }
        _onLoaded(text,false);return;
      }
      ...
      if(mirror){
        ...
        xhrFetch(mirror,30000,function(e2,t2){
          finishLoadBar();
          if(!e2&&t2&&t2.length>100){if(window.AppCache)AppCache.setM3U(rawUrl,t2);_onLoaded(t2,false);}
  ...
  function _onLoaded(t,fromCache){
...
// ── Network monitor — FIX: immediate update on change ─────────────
...
// ── Fullscreen — FIX #7: fallback timer + always call applyExit ───
...
function exitFS(){
  // FIX #7: call applyExitFSState immediately; also try native API
  var fn=document.exitFullscreen||document.webkitExitFullscreen||document.mozCancelFullScreen;
  if(fn)try{fn.call(document);}catch(e){}
  // Fallback: if fullscreenchange doesn't fire within 300ms, apply state manually
  _fsExitFallback=setTimeout(applyExitFSState,300);
}
function toggleFS(){if(isFullscreen)exitFS();else enterFS();}
function _onFsChange(){
  clearTimeout(_fsExitFallback);
  if(!(document.fullscreenElement||document.webkitFullscreenElement)&&isFullscreen)applyExitFSState();
}
...
// ── Connect modal — shown only when no saved server or first run ──
function _setConnectStatus(msg,color){
  if(!Dom.jiotvConnectStatus)return;
  Dom.jiotvConnectStatus.textContent=msg;
  Dom.jiotvConnectStatus.style.color=color||'var(--text-sec)';
}
...
function _closeConnectModal(){
  if(Dom.jiotvConnectModal)Dom.jiotvConnectModal.style.display='none';
  _jioScanInProgress=false;
}

// ── FIX #10: auto-connect with 5s timeout + graceful fallback ─────
async function _connectToServer(serverUrl){
  ...
  var res=await Promise.race([
    client.checkStatus(),
    new Promise(function(resolve){setTimeout(function(){resolve({status:false,reason:'Timeout (5s)'});},5000);})
  ]);
  if(res.status){
    client.logged_in=true;
    JioTVClient.saveUrl(serverUrl);
    return client;
  }
...
async function openJioPortalDirect(){
  ...
  // If already connected and have channels, go straight to portal
  if(jiotvClient&&jiotvClient.logged_in&&jiotvChannels.length>0){
    jpFiltered=jiotvChannels.slice();_jpCurrentSet='';
    saveMode();showJioPortal();return;
  }

  // Try saved URL first, silently
  var saved=JioTVClient.loadSaved();
  if(saved){
    setStatus('Connecting to JioTV…','loading');startLoadBar();
    try{
      var client=await _connectToServer(saved);
      jiotvClient=client;jiotvMode=true;
      await loadJioChannels();finishLoadBar();saveMode();showJioPortal();return;
    }catch(e){
      console.warn('[JioTV] saved URL failed:',e.message);
      JioTVClient.clearSaved();// saved URL no longer works — clear it
    }
  }
  ...
  if(found){
    _setConnectStatus('✅ Found at '+found+' — connecting…','var(--green)');
    try{
      var c=await _connectToServer(found);
      jiotvClient=c;jiotvMode=true;
      _closeConnectModal();
      await loadJioChannels();saveMode();showJioPortal();
    }catch(e){
      _setConnectStatus('❌ Found server but connect failed: '+e.message,'var(--red)');
    }
  }else{
    _setConnectStatus('❌ Not found. Enter URL manually below.','var(--red)');
    if(Dom.jiotvManualUrl)Dom.jiotvManualUrl.focus();
  }
}

// Manual connect button in modal
async function _jioManualConnect(){
  ...
  _setConnectStatus('Connecting to '+raw+'…','var(--gold)');
  if(Dom.jiotvManualBtn)Dom.jiotvManualBtn.disabled=true;
  try{
    var client=await _connectToServer(raw);
    jiotvClient=client;jiotvMode=true;
    _closeConnectModal();
    await loadJioChannels();saveMode();showJioPortal();
  }catch(e){
    _setConnectStatus('❌ '+e.message,'var(--red)');
  }finally{
    if(Dom.jiotvManualBtn)Dom.jiotvManualBtn.disabled=false;
  }
}

// Scan button in modal — re-scan on demand
async function _jioRescan(){
  if(_jioScanInProgress)return;
  _jioScanInProgress=true;
  if(Dom.jiotvScanBtn)Dom.jiotvScanBtn.disabled=true;
  _setConnectStatus('🔍 Scanning LAN…','var(--gold)');
  var found=await JioTVClient.discover(function(msg){_setConnectStatus('🔍 '+msg,'var(--gold)');});
  _jioScanInProgress=false;
  if(Dom.jiotvScanBtn)Dom.jiotvScanBtn.disabled=false;
  if(found){
    if(Dom.jiotvManualUrl)Dom.jiotvManualUrl.value=found;
    _setConnectStatus('✅ Found: '+found,'var(--green)');
  }else{
    _setConnectStatus('❌ Not found on LAN','var(--red)');
  }
}
...
function _jpBuildGrid(){
  ...
  // ResizeObserver — also recalculates when portal becomes visible
...
function _jpSyncPlayingState(){
  if(!Dom.jpGrid)return;
  Dom.jpGrid.querySelectorAll('.jp-tile').forEach(function(t,i){t.classList.toggle('playing',!!(jpActiveChannel&&jpFiltered[i]&&jpFiltered[i].jioId===jpActiveChannel.jioId));});
}
function jpFocusTile(){
  ...
  if(tiles[idx]){tiles[idx].classList.add('focused');tiles[idx].scrollIntoView({block:'nearest',inline:'nearest'});jpFocusRow=Math.floor(idx/Math.max(1,jpGridCols));jpFocusCol=idx%Math.max(1,jpGridCols);}
}
...
function jpMoveFocus(dr,dc){
  ...
  jpFocusRow=newRow;jpFocusCol=Math.max(0,newCol);jpFocusTile();
}
  ...
  _jpLoadPromise=_jpLoadPromise.then(async function(){
    ...
    try{
      ...
      if(!jpPlayer){
        ...
        jpPlayer.configure({
          streaming:{lowLatencyMode:true,inaccurateManifestTolerance:0,bufferingGoal:10,rebufferingGoal:1.5,stallEnabled:true,stallThreshold:1,stallSkip:0.1,retryParameters:{maxAttempts:4,baseDelay:300,backoffFactor:1.5,fuzzFactor:0.3,timeout:15000}},
          drm:{retryParameters:{maxAttempts:3,baseDelay:500,backoffFactor:2,timeout:12000},advanced:{'com.widevine.alpha':{videoRobustness:'HW_SECURE_ALL',audioRobustness:'HW_SECURE_CRYPTO'}}},
        });
      ...
      clearTimeout(_jpPlayTimeoutTimer); // FIX #1: cancel timeout on success

      jpShowOverlay();
      setTimeout(function(){if(jpInPlayer)jpHideOverlay();},OVERLAY_AUTO_HIDE);

      // FIX #4: EPG polling at 30s, not 800ms; stop when hidden
      clearTimeout(jpEpgTimer);
      (function schedEpg(){
        jpEpgTimer=setTimeout(function(){
          if(!jpInPlayer||!Dom.jpPlayerLayer||Dom.jpPlayerLayer.style.display==='none')return;
          jpFetchEpg(ch.jioId).then(schedEpg);
        },EPG_INTERVAL_MS);
      })();
...
async function loadJioChannels(){
  ...
  if(window.AppCache){
    ...
    if(cached&&cached.length>0){
      ...
      setTimeout(_refreshJioBackground,600);return;
    }
  }
  setStatus('Loading JioTV…','loading');startLoadBar();
  try{
    var list=await jiotvClient.getChannelsFormatted();
    _applyJioChannels(list);if(window.AppCache)AppCache.setJioChannels(list);finishLoadBar();
...
async function _refreshJioBackground(){
  try{
    var list=await jiotvClient.getChannelsFormatted();_applyJioChannels(list);
...
async function loadMode(){
  ...
  if(mode==='jiotv'){
    ...
    if(saved){
      try{
        var client=await _connectToServer(saved);
        jiotvClient=client;jiotvMode=true;plIdx=TAB_JIOTV();rebuildTabs();
        await loadJioChannels();saveMode();return;
      }catch(e){JioTVClient.clearSaved();console.warn('[JioTV] restore failed:',e.message);}
    }
  }
...
// ══════════════════════════════════════════════════════════════════
// SETTINGS MODAL — FIX: BACK key closes + restores focus
// ══════════════════════════════════════════════════════════════════
...
window.addEventListener('keydown',function(e){
  ...
  resetSleepTimer(kc); // FIX #3: only resets on qualifying keys
  ...
  if(Dom.jiotvConnectModal&&Dom.jiotvConnectModal.style.display==='flex'){
    ...
    if(k==='Enter'||kc===KEY.ENTER){_jioManualConnect();e.preventDefault();return;}
    return;
  }
...
// tizenhwkey — lowercase for firmware compatibility
